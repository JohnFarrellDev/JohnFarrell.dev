...
export const Minesweeper = () => {
  return (
    <section className="page-center">
      <Title title="Solving Minesweeper" underlineClassName="bg-orange-400" />
      ...
      <p>
        Generating the grid for the game is done by creating a simple 2d array where each inner array represents a row.
        Each element within the inner array represents the state of a particular cell in the grid.
      </p>
      ...
      <p>
        The second step is to ensure that N cells contain a bomb where N is the number of bombs for that specific game.
        An additional rule is that the first cell clicked by a player on starting the game should never contain a bomb.
        Because of the additional rule bomb placement can only be done directly after the first player click. Simply
        every cells position is added to an array except the position of the cell the user clicked. That array is simply
        shuffled using the{' '}
      ...
      <p>
        ...
        shuffling algorithm but simply shows the starting bomb positions and end bomb position (but here you go with my
        lies anyway)
      </p>
      ...
      <p>
        And finally, a functionality that is normally implemented in Minesweeper is the "recursive reveal". If a player
        clicks a cell that contains no neighboring bombs all adjacent cells should be automatically uncovered. If any of
        the adjacent cells also have no neighboring bombs then it's neighbors can be uncovered and so on...
      </p>
      ...
      <p>
        Most of the time when a player is able to with certainty uncover a cell knowing it is not a bomb or place a flag
        on a cell as they know it is a bomb they are utilising{' '}
      ...
      <p>
        By applying these 2 constraints we able to uncovered more of the grid, each time we uncover more of the grid we
        can apply the constraints again (hench the propagation)
      </p>

      <p>
        For someone who is very adept at Minesweeper it could be good practice to play with the constrain propagation
        turned on. This only leaves the more challenging problems to solve and anything that would be trivial is already
        computed away.
      </p>
