...
treb7uchet`,
      ...
      <p>
        The problem for part 1 was too simply find the first and last numerical digit on each line, merge them together
        (1 and 2 becomes 12), do this for each line and sum the total.
      </p>

      <p>
        The solution for this was fairly obvious, loop through each line, within each line loop from the start of the
        string incrementing through each character until you find a number and also do the reverse decrementing from the
        end of the string until you find a number.
      </p>
...
treb7uchet`,
  ...
  'treb7uchet'
      ...
      <p>
        ...
        actually represented a string) we concatenate the value instead of adding. The other important thing to note is
        that Number(row[j]) will be false(y) for any string that does not represent a numerical value as Number('not a
        number') returns NaN which is falsy.
      </p>
...
total => 281`,
      ...
      <p>
        Then as well as checking for a number as we iterate through each line (from start to end) and (end to start) we
        also check if our current substring contains any of the keys in our object mapping.
      </p>
      <p>
      ...
      <p>
        ...
        but I have never used this before, if I have time I will come back to this problem and learn enough to implement
        the solution using a Trie. Also considering my solution runs in about 10ms on my machine with the real input
        there is not a need for a more performant solution in this case.
      </p>
    </section>
  );
}
