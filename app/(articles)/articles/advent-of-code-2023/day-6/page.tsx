...
export default function Day6() {
  return (
    <section className="page-center">
      <Title title="Advent of Code 2023 - Day 6" underlineClassName="bg-orange-400" />
      ...
      <p>
        The problem involved a user with a toy boat that starts with a speed of 0. The boat is equipped with a charging
        button that can be held for as long as the user desires. Each millisecond the button is held, the boat's speed
        increases by 1 mm per millisecond. However, a timer starts counting down as soon as the button is pressed.
        Therefore, holding the button for an extended period could increase the distance traveled, but holding it for
        too long could leave the boat without enough time to cover the required distance. The task was to determine how
        many different times exist at which holding the button down for that duration would result in the boat traveling
        a greater distance than the associated target distance.
      </p>
      ...
      <h3>Part 2</h3>
      <p>
        Part 2 wasn't significantly more challenging than Part 1; the primary change was in how the input was processed.
        Unlike Part 1, the input for Part 2 should not be read removing the spaces between the numbers.
      </p>
      ...
      <p>
        An optimization strategy involves two loops: the first starts at 0, incrementing by 1 until a valid distance is
        found for the minimum time. The second loop starts at the input time, decrementing by 1 until a valid distance
        is found for the maximum time. The difference between the maximum and minimum values, plus one, represents the
        total number of valid solutions for the duration the boat's button could be held. This approach leverages the
        certainty that all values between the minimum and maximum are valid, while those outside the range are not.
      </p>
      <p>
